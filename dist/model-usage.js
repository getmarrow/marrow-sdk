...
exports.modelUsageRequestFacts = modelUsageRequestFacts;
exports.extractModelUsageFromResponse = extractModelUsageFromResponse;
exports.normalizeModelUsageInput = normalizeModelUsageInput;
const count = (v) => typeof v === 'number' && Number.isSafeInteger(v) && v >= 0 ? v : undefined;
...
const label = (v) => typeof v === 'string' && /^[a-zA-Z0-9_.:/@-]{1,160}$/.test(v) && !credential.test(v) ? v : undefined;
const object = (v) => v && typeof v === 'object' && !Array.isArray(v) ? v : {};
const firstCount = (...values) => values.map(count).find(v => v !== undefined);
const path = (v, keys) => keys.split('.').reduce((item, key) => object(item)[key], v);
// Exact request endpoints establish billing-host provenance. Never infer it from a model name or response shape.
function providerEndpoint(rawUrl) {
    try {
        ...
        if (u.hostname === 'generativelanguage.googleapis.com' && /^\/v1(?:beta)?\/models\/[a-zA-Z0-9_.-]+:generateContent$/.test(u.pathname))
...
async function modelUsageRequestFacts(input, init) {
    try {
...
async function extractModelUsageFromResponse(rawUrl, response, request = Promise.resolve({})) {
    const endpoint = providerEndpoint(rawUrl);
    if (!endpoint || !response.ok || !/\bjson\b/i.test(response.headers.get('content-type') || ''))
        return null;
    // A cross-host redirect cannot inherit the original endpoint's provenance.
    if (response.url && (!providerEndpoint(response.url) || new URL(response.url).hostname !== new URL(rawUrl).hostname))
        return null;
    let data;
    try {
        data = object(await response.clone().json());
    }
    catch {
        return null;
    }
    ...
    const dimensions = {};
    if (facts.modality)
        dimensions.modality = facts.modality;
    if (endpoint.region)
        dimensions.region = endpoint.region;
    const actualTier = usage.service_tier ?? data.service_tier ?? path(data, 'response.service_tier');
    if (actualTier === 'default' && endpoint.provider === 'openai')
        dimensions.tier = 'standard';
    else if (['standard', 'priority', 'flex', 'fast', 'batch'].includes(String(actualTier)))
        dimensions.tier = String(actualTier);
    if (['global', 'us'].includes(String(usage.inference_geo)))
        dimensions.region = String(usage.inference_geo);
    let semantics;
    if (endpoint.provider === 'openai') {
        ...
        if (five !== undefined && hour !== undefined) {
            ...
            if (five + hour !== writes)
                return null;
            if (five > 0 && hour === 0)
                dimensions.cache_ttl = '5m';
            else if (hour > 0 && five === 0)
                dimensions.cache_ttl = '1h';
            else if (five > 0 && hour > 0)
                dimensions.cache_ttl = 'mixed_unresolved';
        }
    }
    const total = firstCount(usage.total_tokens, usage.totalTokenCount, usage.totalTokens);
    if (![input, output, cached, writes, total].some(v => v !== undefined && v > 0))
        return null;
    ...
    if (cached === undefined || endpoint.provider === 'anthropic' && writes === undefined)
        semantics = undefined;
    return {
...
function normalizeModelUsageInput(input, sanitize) {
    const body = {};
    const strings = ['agent_id', 'session_id', 'workflow_id', 'decision_id', 'task_type', 'action_type', 'source', 'marrow_intervention'];
    for (const key of strings) {
        const v = input[key];
        if (typeof v === 'string' && v.trim())
            body[key] = sanitize(v).slice(0, 180);
    }
    for (const key of ['provider', 'model', 'billing_host', 'usage_event_id', 'baseline_usage_id', 'comparison_id', 'task_fingerprint', 'constraints_fingerprint']) {
        const v = label(input[key]);
        if (v && sanitize(v) === v)
            body[key] = v;
    }
    ...
    for (const key of ['cost_usd', 'latency_ms', 'estimated_cost_saved_usd', 'estimated_minutes_saved']) {
        const v = input[key];
    ...
    for (const key of Object.keys(enums))
        if (enums[key].includes(String(input[key])))
            body[key] = input[key];
    for (const key of ['success', 'coverage_complete', 'overhead_complete'])
        if (typeof input[key] === 'boolean')
            body[key] = input[key];
    if (typeof input.occurred_at === 'string' && input.occurred_at.length <= 64 && Number.isFinite(Date.parse(input.occurred_at)))
