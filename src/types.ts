...
export type MarrowDecisionSourceClient =
  | 'claude-code'
  | 'cursor'
  | 'windsurf'
  | 'openclaw'
...
export interface MarrowGuardedRunOptions<T> {
  ...
  project?: MarrowGovernanceProject & { fingerprint?: string; harness?: string };
...
export interface MarrowAcquireResourceLeaseResult {
  ...
  conflict?: { holder_agent_id: string; expires_at: string };
  exact_next_action: string;
}
...
export interface MarrowReplayComparisonInput {
  ...
  baseline: { label?: string; decisionId: string };
  candidate: { label?: string; decisionId: string };
...
export interface MarrowFirstValueResult {
  ...
  next_action: { endpoint: string; method: string; action: string; reason?: string };
