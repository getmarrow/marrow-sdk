...
test('observation lifecycle never invokes stateful action methods or intercepts SDK control-plane requests', async () => {
  ...
  try {
    const marrow = new MarrowClient(process.env.MARROW_API_KEY, { durableEventSpool: false });
    const interceptions = { before: 0, after: 0 };
    marrow.beforeAction = async () => {
      interceptions.before += 1;
      return { ok: true };
    };
    marrow.afterAction = async () => {
      interceptions.after += 1;
      return { ok: true };
    };
...
test('orient unavailable response includes normalized repair and update guidance', async () => {
  ...
  try {
    ...
    assert.equal(orient.client_update.installed_version, sdkVersion);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test('SDK identifies its package version and exposes the server update advisory', async () => {
  ...
  try {
    ...
    assert.equal(capturedHeaders['X-Marrow-Package-Version'], sdkVersion);
...
test('SDK runtime identity and authorization semantics are model and harness neutral', async () => {
  ...
  try {
    ...
    for (const [configuredClient, expectedClient] of expectedClients) {
      ...
      assert.equal(headers['X-Marrow-Package-Version'], sdkVersion);
...
test('agentRuntime returns structured low-risk guidance when the live read is unavailable', async () => {
  ...
  try {
    ...
    assert.equal(runtime.client_update.installed_version, sdkVersion);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
